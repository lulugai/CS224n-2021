...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + hidden_size, self.hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden_reshape = torch.cat((last_hidden[0, :, :], last_hidden[1,:,:]), 1)
        init_decoder_hidden = self.h_projection(last_hidden_reshape)
        last_cell_reshape = torch.cat((last_cell[0], last_cell[1]), 1)
        init_decoder_cell = self.c_projection(last_cell_reshape)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=-1))#(b,T,1)
