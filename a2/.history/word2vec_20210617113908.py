...
def negSamplingLossAndGradient(
    ...
    loss += -np.log(score_o)
    gradCenterVec += np.dot(score_o - 1, outsideVectors[outsideWordIdx, :])
    gradOutsideVecs[outsideWordIdx, :] += np.dot(score_o - 1, centerWordVec)

    u_k = outsideVectors[negSampleWordIndices]
    score_k = sigmoid(-np.dot(u_k, centerWordVec))#(N,)
    loss += -np.sum(np.log(score_k))
    gradCenterVec += np.dot(u_k.T, 1 - score_k)
