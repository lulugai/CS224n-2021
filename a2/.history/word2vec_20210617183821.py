...
def naiveSoftmaxLossAndGradient(
    ...
    score = softmax(outsideVectors.dot(centerWordVec))
    tmp = score.copy()
    tmp[outsideWordIdx] -= 1
    loss = -np.log(score)[outsideWordIdx]
    gradCenterVec = -(outsideVectors[outsideWordIdx, :] - np.dot(outsideVectors.T, score))
    # gradCenterVec = np.dot(outsideVectors.T, tmp)
    gradOutsideVecs = np.dot(tmp[:, np.newaxis], centerWordVec[np.newaxis, :])
...
def negSamplingLossAndGradient(
    ...
    score_o = sigmoid(np.dot(outsideVectors[outsideWordIdx,:], centerWordVec))#(1,)
    loss -= np.log(score_o)
    gradCenterVec += np.dot(score_o - 1.0, outsideVectors[outsideWordIdx, :])
    score_no = sigmoid(-np.dot(outsideVectors[outsideWordIdx,:], centerWordVec))#(1,)
    gradOutsideVecs[outsideWordIdx] += (score_o - score_no) * centerWordVec

    u_k = outsideVectors[negSampleWordIndices]
    score_k = sigmoid(-np.dot(u_k, centerWordVec))#(N,)
    loss -= np.sum(np.log(score_k))
    gradCenterVec += np.dot(u_k.T, 1.0 - score_k)
    gradOutsideVecs[negSampleWordIndices] += np.outer((1 - score_k), centerWordVec)#(N,D)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterWord_idx = word2Ind[currentCenterWord]
    cur_cWV = centerWordVectors[currentCenterWord_idx]#(D,)
    for outsideWord in outsideWords:
        loss1, gradCenterVecs1, gradOutsideVectors1 = word2vecLossAndGradient(cur_cWV, word2Ind[outsideWord], outsideVectors, dataset)
        loss, gradCenterVecs, gradOutsideVectors += loss1, gradCenterVecs1, gradOutsideVectors1
    ### END YOUR CODE
