...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(n_features*self.embed_size, hidden_size)))
        ...
        # self.embed_to_hidden_weight = nn.Linear(self.embed_size * self.n_features, self.hidden_size)
        # nn.init.xavier_uniform_(self.embed_to_hidden_weight.weight, gain=1)
        # self.embed_to_hidden_bias = nn.Linear(self.hidden_size, 1)
        # nn.init.uniform_(self.embed_to_hidden_bias.weight)
        # self.dropout = nn.Dropout(p = self.dropout_prob)
        # self.hidden_to_logits_weight = nn.Linear(self.hidden_size, self.n_classes)
        # nn.init.xavier_uniform_(self.hidden_to_logits_weight.weight, gain=1)
        # self.hidden_to_logits_bias = nn.Linear(self.n_classes, 1)
        # nn.init.uniform_(self.hidden_to_logits_bias.weight)
    ...
    def embedding_lookup(self, w):
        ...
        ### TODO:
        ###     1) For each index `i` in `w`, select `i`th vector from self.embeddings(num_words, embedding_size)
        ...
        # embed = nn.Embedding(self.embeddings.shape[0], self.embed_size)
        # embed.weight = nn.Parameter(torch.tensor(self.embeddings))
        # x = embed(w)
        # x = x.view(x.size()[0], -1)        
        x = self.embeddings[w].view(w.size()[0], -1)#b x n_feature x embed_size -> bxT*C
    ...
    def forward(self, w):
        ...
        h = nn.functional.relu(x.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h = self.dropout(h)
