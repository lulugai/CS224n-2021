...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]
        self.buffer = self.sentence.copy()#not modify the sentance object
        self.dependencies = []
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            dependency = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], dependency))
        elif transition == 'RA':
            dependency = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], dependency))
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for pp, transition in zip(minibatch_parse, transitions):
            pp.parse([transition])
            if len(pp.buffer) == 0 and len(pp.stack) == 1:
                unfinished_parses.remove(pp)
        # for i in range(len(transitions)):
        #     minibatch_parse[i].parse([transitions[i]])
        #     if not minibatch_parse[i].buffer and len(minibatch_parse[i].stack) ==1:
        #         unfinished_parses.remove(minibatch_parse[i])
